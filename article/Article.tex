...
A method published recently that offers improved
...
later supports arbitary kernel shapes and pixel types as well as any
choice of rank.
...
fact it is also posible to apply the decomposition discussed in
...
A more efficient strategy was proposed in \cite{Crow84} for rectangular
kernels. This approach first computes an accumulator image in which
each $i,j$ is repaced by .. This accumulator image may be computed
recursively using local neighborhood values. The mean of rectangles of
any size may then be computed using accumulated values at the
rectangle corners. Figures \ref{fig:accum} amd \ref{fig:accumMean}
illustrate the 2D case and the update formulas are provided in
Equations \ref{eq:accum} and \ref{eq:accumMean}.
...
\begin{equation}
\label{eq:accumMean}
m_{i,j} = \frac{1}{wh} (b_{i+w/2,j+h/2} + b_{i-w/2,j-h/2} - b_{i-w/2,j+h/2} + b_{i+w/2,j-h/2})
\end{equation}
...
\caption{Computation of mean using the accumulated image - lue at $(i,j)$ computed using Equation \ref{eq:accumMean}.\label{fig:accumMean}}
\end{figure}
...
The {\em fastRankMean} packge using the sliding window method to
...
performance sor kernel radii greater than 3. The execution times of
box convolution should be relatively constant with kernel size, but an
obvious increase is clear. This is attributable to the process of
dealing with the boundary conditions, which utilizes shaped
neighborhood filters. This aspect of the implementation needs further work
...
\hline
\hline
1   &    0.0109048   &    0.0233631 \\
2   &    0.022008    &    0.0245971 \\
3   &    0.0434739   &    0.0263161 \\
4   &    0.0780701   &    0.0283061 \\
5   &    0.129174    &    0.0310797 \\
6   &    0.199876    &    0.0344833 \\
7   &    0.289754    &    0.0382629 \\
8   &    0.407095    &    0.043121 \\
9   &    0.552862    &    0.0489335 \\
10  &    0.728667    &    0.0555441 \\
15  &    2.17149     & 0.107776 \\
20  &    4.83458     & 0.199508 \\
25  &    9.12699     & 0.372637 \\
\hline
...
\end{tabular}
\caption{Execution times for direct and box convolution implementations applied to the $256 \times 256$ cthead image.\label{tab:perfBoxConv}}
\end{table}
